...
Rails.application.configure do
  ...
  config.cache_classes = ENV["CODE_RELOAD"] == "0"

  # Do not eager load code on boot.
  config.eager_load = ENV["EAGER_LOAD"] == "1" || ENV["CODE_RELOAD"] == "0"
  ...
  if Rails.root.join("tmp", "caching-dev.txt").exist?
    ...
    config.graphql_fragment_cache.store = :memory_store
  ...
  config.action_mailer.file_settings = {location: Rails.root.join("tmp/mailbox")}
